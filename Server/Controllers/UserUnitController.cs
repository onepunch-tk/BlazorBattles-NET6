    ...
    public class UserUnitController : ControllerBase
    {
        ...
        public async Task<IActionResult> ReviveArmy()
        {
            ...
            var userUnits = await _context.UserUnits
                .Where(unit => unit.UserId.Equals(user.Id))
            ...
            if (user.Bananas < bananaCost)
                return BadRequest("Not enough bananans! You Need 1000 bananas to revive your army.");
        ...
        {
            var unit = await _context.Units.FirstOrDefaultAsync(u => u.Id.Equals(unitId));
            var user = await _utilityService.GetUser();
